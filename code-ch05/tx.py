...
class Tx:
    ...
    def parse(cls, s, testnet=False):
        ...
        # Edmund - this part is for parsing the version
        version = little_endian_to_int(s.read(4))
        
        # Edmund - this part is for parsing the input of a transaction
        num_inputs = read_varint(s)
        inputs = []
        for _ in range(num_inputs):
            inputs.append(TxIn.parse(s))
        # Edmund - this part is for parsing output of a transaction    
        num_outputs = read_varint(s)
        outputs = []
        for _ in range(num_outputs):
            outputs.append(TxOut.parse(s))
        
        # Edmund - this part is for parsing the locktime
